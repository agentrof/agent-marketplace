...
1. Read every package's dependency field; confirm the order never places a
   package before one it depends on (cycle and graph-shape rules live in
...
even when a richer package carries more user value, because its failure
teaches more, earlier, than any other package's success. Record it in the
priority field: "P0: walking skeleton".

## Step 3: Risk Pass

Among the packages the dependency pass leaves free, order by what a
failure would invalidate:

- First the package whose failure would force the most other packages to
  be re-planned: count its outgoing dependency edges plus the packages
  whose Definition of Done assumes it works.
- De-risk or spike the high-uncertainty package first when its assumptions
  gate others; a surprise there discovered late re-plans the whole tail.
- A risky package that gates nothing waits its turn; risk alone does not
  jump the queue.
...
package costs. The cosmetic tail (polish, copy, layout refinement) goes
last as named packages; it is still on the backlog, never silently
dropped.
...
After the skeleton, suppose three packages are free: WP-02 Decide a
report, WP-06 Foreign-currency expenses, WP-08 Find reports.

| Package | Value if shipped | Risk retired | Size |
|---|---|---|---|
...
assumption gates the export package; if the assumption fails, WP-03's
...
- Revisit the deferred list at every checkpoint: reinstate the item as a
  package, re-affirm its reason, or drop it by the owner's explicit
  decision. A deferred list untouched across two checkpoints is a smell,
  not a steady state.
- Deferral is scope negotiation, not ordering: a package ordered last is
