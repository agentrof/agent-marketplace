...
class PmoHookTests(unittest.TestCase):
    ...
    def install_codex_agent(self, name: str):
        ...
        path.write_text(
            "# Generated by Agent Marketplace software-engineering-team; do not edit by hand.\n"
            f'name = "{name}"\n',
            encoding="utf-8",
        )
    ...
    def test_session_start_injects_resume_context(self):
        ...
        self.assertIn("AGENT_MARKETPLACE_HOOKS_ACTIVE: project-management-office", context)
        self.assertIn("AGENT_MARKETPLACE_PMO_READY: project-management-office", context)
        ...
        launcher = Path(self.env["AGENT_MARKETPLACE_HOME"]) / "bin" / "pmo_cli.py"
        self.assertTrue(launcher.is_file())
        dashboard_module = Path(self.env["AGENT_MARKETPLACE_HOME"]) / "bin" / "pmo_dashboard.py"
        self.assertTrue(dashboard_module.is_file())
        dashboard_index = Path(self.env["AGENT_MARKETPLACE_HOME"]) / "dashboard" / "index.html"
        self.assertTrue(dashboard_index.is_file())

    def test_session_start_with_failed_bootstrap_is_not_ready(self):
        database = Path(self.env["AGENT_MARKETPLACE_HOME"]) / "pmo.db"
        ...
        self.assertIn("AGENT_MARKETPLACE_HOOKS_ACTIVE: project-management-office", context)
        self.assertIn("AGENT_MARKETPLACE_UPGRADE_REQUIRED_BLOCKED", context)
        self.assertNotIn("AGENT_MARKETPLACE_PMO_READY", context)
    ...
    def attempts(self):
        import sqlite3
        con = sqlite3.connect(Path(self.env["AGENT_MARKETPLACE_HOME"]) / "pmo.db")
    ...
    def test_guard_denies_db_writes_and_allows_others(self):
        db = Path(self.env["AGENT_MARKETPLACE_HOME"]) / "pmo.db"
    ...
    def test_guard_locks_normal_pmo_commands_when_upgrade_is_required(self):
        sessions = Path(self.env["AGENT_MARKETPLACE_HOME"]) / "sessions"
        sessions.mkdir(parents=True, exist_ok=True)
        state = sessions / (hashlib.sha256(b"s1").hexdigest() + ".json")
        state.write_text(json.dumps({
            "session_id": "s1",
            "pmo_ready": False,
            "upgrade_status": "AGENT_MARKETPLACE_UPGRADE_REQUIRED_READY",
        }), encoding="utf-8")
        code, _, err = run_hook("hook_guard_db.py", self.payload(
