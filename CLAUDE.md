...
14. **Deep primitives epistemic loop**: `create-skill`, `create-agent`, `update-skill`, `update-agent` run an iterative multi-persona reasoning loop before writing. Fixed intensity 10 iterations, 10 questions, 10 personas, 10 critics, early-exit on consensus. No tiers. Loop output persists under `.run/<uuid>/artifacts/iter-N/`. Simulation-based; true parallel diversity is Phase 2. **Canonical specs live at `.claude/skills/create-agent/references/personas.md` and `.claude/skills/create-agent/references/loop-spec.md`**; all four deep primitives read these at runtime, no duplicates.
15. **Generated deep agents**: agents created or updated with deep mode enabled (`create-agent --deep` or `update-agent` deep toggle) carry a step `0` preamble in their `## Flow` that reads the two canonical spec files and runs the same loop on every invocation before any task step. Opt-in only; non-deep generated agents skip the loop entirely.
16. **Primitives provided**: `create-agent`, `create-skill`, `list-agents`, `list-skills`, `update-agent`, `update-skill`.
17. **Adding entries**: invoke `create-skill` or `create-agent` in dialog; the skill asks for missing inputs, runs the epistemic loop, presents synthesized design for approval, then writes to `.claude/`. `create-agent` asks whether the new agent should itself be deep.
18. **Scope**: Phase 1 is Claude-only and ships only the six primitives plus rules. Orchestration, inter-component messaging, workflow chaining, multi-IDE shims, registry, delete operations, runner-based parallelism, and example user-domain components are deferred.
