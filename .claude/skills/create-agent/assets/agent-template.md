...
{{DEEP_SECTION}}1. Open run workspace per CLAUDE.md section 13. Generate UUID4, create `.run/<uuid>/META.md` (`status: pending`, `component: {{AGENT_ID}}`, `kind: agent`) and `.run/<uuid>/artifacts/`. Print `Task <uuid-short> started. Output at .run/<uuid-full>/`.
2. {{STEP_1}}
3. {{STEP_2}}
