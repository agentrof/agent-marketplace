...
PROVENANCE = ".agentrof-package.json"
...
def write_provenance(target: Path, component: str, host: str, version: str) -> None:
    files = {}
    for path in sorted(candidate for candidate in target.rglob("*") if candidate.is_file()):
        if path.name == PROVENANCE or is_python_cache(path):
            continue
        files[path.relative_to(target).as_posix()] = hashlib.sha256(
            path.read_bytes()
        ).hexdigest()
    (target / PROVENANCE).write_text(json.dumps({
        "schema_version": 1,
        "component": component,
        "host": host,
        "version": version,
...
def build_plugin(
    ...
    write_provenance(target, source.name, host, version)
