...
2. After any decision write, re-render the index: `"$RUN" run "$TEAM" scripts/vault_check.py render-decisions --vault workspace/docs`.
3. Consistency check, mechanical: artifact_check green on every touched doc; landscape_check exit 0 over the tree (every Components decision link targets an existing, non-superseded note; every Target delta cites a note; every engagement carries a valid status line; ids are unique); and the vault check scoped to this subtree (`"$RUN" run "$TEAM" scripts/vault_check.py check --vault workspace/docs --scope solution-design`).
