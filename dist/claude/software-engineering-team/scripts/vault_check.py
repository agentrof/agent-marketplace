...
CATALOG_LINK_LIMIT = 100
CATALOG_FIXED_LINKS = 1
        ...
        if is_os_metadata_path(rel):
            ...
            if not safe:
                findings.append(Finding(
                    ...
                    "operating-system metadata must be a regular single-link file",
                    "remove the alias or special file; metadata exclusions never waive filesystem safety"))
            continue
...
def relation_projection(vault: Vault) -> tuple[dict[str, str], dict[str, str]]:
    ...
    edge_page_size = min(page_size, CATALOG_LINK_LIMIT - CATALOG_FIXED_LINKS)
    ...
    for target, edges in sorted(grouped.items()):
        ...
        if len(edges) <= inline_max:
            ...
            pages = [edges[index:index + edge_page_size]
                     for index in range(0, len(edges), edge_page_size)]
...
def check_generated_views(vault: Vault, findings: list[Finding]) -> None:
    ...
    for rel, note in sorted(vault.notes.items()):
        ...
        if len(links) > CATALOG_LINK_LIMIT:
            findings.append(Finding(
                "error", rel, 1, "generated_views",
                f"generated navigation catalog exceeds {CATALOG_LINK_LIMIT} links",
                "re-render it; structural and relation catalogs shard at"
                f" {CATALOG_LINK_LIMIT} links per page"))
