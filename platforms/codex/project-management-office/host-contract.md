...
- For the upgrade entry, present status, plan approval, apply approval, and
  recovery approval through one `request_user_input` popup at a time. A
  blocked status lists the ordered clearing actions and stops without
  mutation. A completed status requires a new task/session.
