...
12. **One source, many harnesses.** The tree under `plugins/` is the
    only authored surface. Everything other harnesses install (the
    Cursor and Codex registries, per-plugin manifests, transformed
    agent trees, hooks manifests, skill policy files, the derived
    context-file template, the runtime data file and the capability
    matrix block) is generated by `tools/generate.py` from the single
    policy file `tools/data/harnesses.json`, where every per-harness
    value lives. Generated files are never hand-edited; they are proven
    by renderer construction plus byte-drift equality (`harness_drift`,
    `make generate-check`), and drift is CI-red. Adding a harness whose
    needs fit the closed action vocabulary is a config edit, not a
    Python change.

## Repository layout

- `.claude-plugin/marketplace.json`: the catalog registry.
- `.cursor-plugin/` and `.agents/plugins/`: generated harness
  registries (Cursor, Codex); see [harnesses.md](harnesses.md).
...
- `tools/`: validator, counts injector, harness generator, scaffolder
  and their tests; `tools/data/harnesses.json` is the per-harness
  policy file, validated like any other policy artifact.
