...
2. Start the server as a BACKGROUND process: `"$PMO" dashboard
   ...
   URL from that first line instead.
3. Reply to the user with the clickable link, exactly this shape:
   "Control Tower is running: http://127.0.0.1:<port>/". Add one line:
   the server keeps running in the background and refreshes itself; stop
   it by killing the background process.
4. Never write to the database beyond `init-db` and `sync-launcher`;
   Control Tower itself is read-only by construction.
