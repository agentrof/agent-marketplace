...
3. Every lane start and every merge is an explicit user choice, asked
   through a choice gate. Offer exactly: Approve / Skip /
   Pause.
...
- Present exactly in this shape: "WP-03 and WP-05 can start; ownership
  ...
  flight." The human approves each lane individually through a choice
  gate, one question per lane, the SHARES holdback
...
- Print the handoff, verbatim shape:
  ...
  1. Open a NEW host session in that directory using the active host
     contract's worktree handoff mechanism.
  2. Say: "deliver: WP-<nn>"
...
- The human merges the pull request (merging is a human act; the
  merge-here-or-on-platform choice is asked through a choice gate).
  Then, on the primary checkout: pull main; run the configured
