...
3. Stop at every gate and wait for explicit user choice, asked through
   ...
   is unavailable, stopping for the typed reply (options with
...
- Present the preview; the user picks a direction through the
  ...
  difference in the description; popup unavailable: a
  numbered option list) or requests different emphases (re-run
...
- The user approves the refined preview through the AskUserQuestion
  popup (unavailable: a numbered option list). The approved file IS the
  specification; no separate spec
  document exists.
