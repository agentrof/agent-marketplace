#!/usr/bin/env python3
"""Read-only localhost dashboard over the PMO central database.
...
skills and commands is scanned live off the Claude plugin install records
on disk; usage facts (projects, work orders, tasks, attempts, findings) are
read out of the database. Stdlib only.
"""
...
EVENT_LIMIT_MAX = 500
# Old action strings survive in the append-only audit log; match both.
SESSION_ENDED_ACTIONS = (
    "session_ended_with_active_work_order",
    "session_ended_with_active_run",
)
...
def scan_install(install_path: Path) -> dict:
    ...
    skills = []
    for skill_file in sorted((install_path / "skills").glob("*/SKILL.md")):
...
def scan_catalog() -> dict:
    ...
    manifest declares a dependency on pmo."""
    ...
    for key, records in (registry.get("plugins") or {}).items():
        ...
        for entry in sorted(records, key=lambda r: r.get("lastUpdated", ""),
            ...
            is_backbone = manifest.get("name") == "pmo" or plugin_name == "pmo"
            is_team = "pmo" in (manifest.get("dependencies") or [])
...
def api_head(params) -> dict:
    if not pmo_cli.db_path().is_file():
        return {"head_id": 0, "db_present": False, "schema_version": None}
    con = open_ro()
    try:
        version = con.execute("PRAGMA user_version").fetchone()[0]
        head = 0
        if version <= pmo_cli.SCHEMA_VERSION:
            try:
                ...
                head = 0
        return {"head_id": head, "db_present": True, "schema_version": version}
    finally:
        con.close()
...
def wo_summary(con, order: dict) -> dict:
    ...
    dangling = (order["status"] in pmo_cli.ACTIVE_WO_STATUSES and last is not None
                and last["action"] in SESSION_ENDED_ACTIONS)
    return {
...
def serve(host: str = "127.0.0.1", port: int = 8787,
    ...
    print(f"pmo: dashboard on {url} (read-only; Ctrl+C stops it)")
    if open_browser:
        webbrowser.open(url)
    try:
...
def main(argv: list[str] | None = None) -> int:
    import argparse
    parser = argparse.ArgumentParser(description="PMO read-only dashboard.")
