...
def cmd_checkpoint(args) -> int:
    ...
    render_args = argparse.Namespace(
        ...
        out=str(Path(args.docs_dir) / "backlog.md"),
    )
    code = cmd_render_backlog(render_args)
    if code != 0:
        return code
    render_args.out = str(Path(args.docs_dir) / "quality-ledger.md")
    return cmd_render_ledger(render_args)
...
def cmd_render_backlog(args) -> int:
    ...
    for epic, extra in groups:
        ...
        for story in stories:
            ...
            depends = ", ".join(deps_by_item.get(story["id"], [])) or "none"
