...
- Cadence: the rhythm of checkpoints. This flow's natural cadence is the
  merge checkpoint after each story; forecasts count checkpoints, not
  calendar days, unless the owner supplies dates.
- Throughput: stories marked done per checkpoint interval, read straight
  from the backlog's status fields.
- Cycle time: how long one story takes from leaving ready to being
  marked done at its merge checkpoint.

## Forecasting From the Backlog

1. Count the stories marked done and the intervals they took; that ratio
   is the observed throughput.
2. Divide the remaining non-deferred stories by the observed throughput;
...
4. State the assumptions inside the answer: stories stay one review unit
...
5. With fewer done stories than it takes to see variation, say the sample
