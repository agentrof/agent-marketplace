...
3. Stop at every gate and wait for explicit user choice, asked through
   a choice gate (options with tradeoffs, recommended
   first).
...
- Present the preview; the user picks a direction through a choice gate
  (one option per direction, its axis of
...
- The user approves the refined preview through a choice gate. The
  approved file IS the specification; no separate spec
  document exists.
