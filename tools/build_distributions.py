...
def compose_project_instructions(
    root: Path, source: Path, target: Path, host: str,
) -> None:
    """Compose the one generated project instruction template for a team."""
    ...
    host_delta = (
        root / "platforms" / host / "_team" / "overlay" / "templates"
        / "project-instructions" / "host.md"
    )
    missing = [path for path in (common, team, host_delta) if not path.is_file()]
    if missing:
        raise ValueError(
    ...
    owner = (
        f"<!-- generated by agent-marketplace {source.name} for {host}; "
        "do not edit by hand -->"
    )
    sections = [
    ...
    if size > max_bytes or lines > max_lines:
        raise ValueError(
            f"{source.name}/{host} generated project instructions exceed cap: "
...
def build_plugin(
    ...
    compose_project_instructions(root, source, target, host)
    if source.name == "project-management-office":
        teams = sorted(
