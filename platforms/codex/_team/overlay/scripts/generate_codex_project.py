...
BLOCK_START_RE = re.compile(
    r"<!-- agent-marketplace:([a-z0-9]+(?:-[a-z0-9]+)*):codex:start -->"
)
...
def block_markers(team: str) -> tuple[str, str]:
    return (
        f"<!-- agent-marketplace:{team}:codex:start -->",
        f"<!-- agent-marketplace:{team}:codex:end -->",
    )
...
def merge_agents_md(existing: str, block: str, team: str) -> str:
    ...
    if start == -1 or end == -1 or end < start:
        raise ValueError("AGENTS.md has an incomplete Agent Marketplace managed block")
    end += len(end_marker)
